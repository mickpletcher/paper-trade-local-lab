...
Strategy context includes positions and pending quantities. Reversals cancel stale opposite orders, and exits cannot exceed inventory. Valuation preserves flat run cash. Alpaca refresh validates responses, retries transient failures, and rejects incomplete symbol sets. Moving average crossover is the only built in strategy. Backtests remain single symbol.
...
GitHub Actions runs Ruff, formatting, strict Mypy, lock drift, and tests with an 88 percent coverage floor on runner Python 3.11, 3.13, and 3.14. A separate job builds and health checks the Python 3.14 container. CI also validates packages, governance, dependencies, releases, and GHCR publishing. Protected `main` requires strict checks and squash merges.
